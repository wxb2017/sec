...
int64 nChainStartTime = 1507860814;
...
const string strMessageMagic = "sharedecologicalchain Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in sharedecologicalchain
    bool fStrictPayToScriptHash = true; // Always active in sharedecologicalchain
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && POW_CUTOFF_BLOCK > 0 && nHeight > POW_CUTOFF_BLOCK)
        return DoS(100, error("AcceptBlock() : No PoW block allowed anymore (height = %d)", nHeight));

    if ((!IsProofOfWork() && nHeight < POS_START_BLOCK) || (!IsProofOfWork() && POS_START_BLOCK==-1))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "sharedecologicalchain", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "CBigNum vector sharedecologicalchain empty mapBlockIndex sharedecologicalchain sharedecologicalchain";
        CTransaction txNew;
        ...
        block.nNonce   = !fTestNet ? 2671445 : 279540;
       ...
       // printf("block.nNonce = %u \n", block.nNonce);
		block.print();
        assert(block.hashMerkleRoot == uint256("0x48ea00b0fd2d7f8e71d214387cfb57f52cf9243532875f7c417b0fa480fbb825"));
...
unsigned char pchMessageStart[4] = { 0x42, 0x88, 0x80, 0x25 };
